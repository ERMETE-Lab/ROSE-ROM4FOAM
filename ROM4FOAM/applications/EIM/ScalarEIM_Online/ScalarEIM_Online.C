...
)
{
    ...
    {
        source[zz]= field_to_interpolate[magic_cellsID[zz]];
    }
}
...
#include "setRootCase.H"
    ...
    PtrList<volScalarField> MagicFunctions (mfNumber);
...
#include "createMesh.H"
        ...
        for (label ii=0; ii< MagicFunctions.size(); ++ii)
        {
            for(label jj=0; jj< MagicFunctions.size(); ++jj)
            {
                if (jj<=ii)
                {
                    BMatrix(ii,jj)=MagicFunctions[jj][(MagicCellsID[ii])];
                }
                else
                {
                    BMatrix(ii,jj)=0;
...
#include "CreateMesh.H"
            ...
            forAll(timeDirs, timeI)
            {
                ...
                scalarField source;
                assembleSourceTerm(source,FieldToInterpolate,tmp_MagicCellsID);
