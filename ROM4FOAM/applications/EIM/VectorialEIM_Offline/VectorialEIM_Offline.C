...
//- update B matrix, specialised for volVectorField
void updateBmatrix
(
    scalarSquareMatrix& BMatrix,
    const PtrList<GeometricField<vector, fvPatchField, volMesh>>& magic_functions,
...
)
{
    ...
    {
        BMatrix(mf_size-1,jj)=(magic_functions[jj][(magic_cellsID[mf_size-1])])&(magic_functions[mf_size-1][magic_cellsID[mf_size-1]]);
...
#include "setRootCase.H"
    ...
    //get EIM_Offline parameters (field name, list of folders, maxBasis, error) from MORsolverDict
...
#include "CreateMesh.H"
    ...
    PtrList<volVectorField> MagicFunctions;
    labelList MagicCellsID;

    // definition of the variables needed for the loop

    scalar max_L_infinity_norm=SMALL;
    scalar vectorMagnitudeAtMagicPoint=SMALL;
    label tmp_CellID=0;
    label iterIndex=0;
    ...
    forAll (vectorSnapshotsList, snapI)
    {
        ...
        if(max_L_infinity_norm< MOR::L_infinity_norm(vectorSnapshotsList[snapI]))
        {
            max_L_infinity_norm = MOR::L_infinity_norm(vectorSnapshotsList[snapI]);
    ...
    forAll(vectorSnapshotsList[generatingFunction], CellI)
    {
        if (mag(vectorMagnitudeAtMagicPoint)< mag((vectorSnapshotsList[generatingFunction])[CellI]))
        {
            vectorMagnitudeAtMagicPoint=mag((vectorSnapshotsList[generatingFunction])[CellI]);
            tmp_CellID=CellI;
        }
    }
    ...
    (
        ...
        (
            IOobject
            (
                vectorSnapshotsList[generatingFunction].name(),
                vectorSnapshotsList[generatingFunction].time().timeName(),
                mesh,
                IOobject::NO_READ,
                IOobject::NO_WRITE
            ),
            vectorSnapshotsList[generatingFunction]/vectorMagnitudeAtMagicPoint,
            fvPatchField<vector>::calculatedType()
        )
    );
    MagicCellsID.append (tmp_CellID) ;
    ...
    InterpolationCoeffList.setSize(vectorSnapshotsList.size());
    ...
    while ( (max_relative_error>desidered_error) && (iterIndex<=maxBasis))
    {
        ...
        vectorMagnitudeAtMagicPoint=SMALL;

        volVectorField* tmp_residual_Ptr= new GeometricField<vector, fvPatchField, volMesh>
        (
            vectorSnapshotsList[0]
        );
        ...
        forAll (vectorSnapshotsList, snapI)
        {
            ...
            InterpolationCoeffList[snapI][coeffIndex] = (vectorSnapshotsList[snapI][MagicCellsID[coeffIndex]])&(MagicFunctions[coeffIndex][MagicCellsID[coeffIndex]]);
            ...
            (
                IOobject
                (
                    vectorSnapshotsList[snapI].name() + "EIMInterpolation",
                    vectorSnapshotsList[snapI].time().timeName(),
                    mesh,
                    IOobject::NO_READ,
                    IOobject::NO_WRITE
                ),
            ...
            (
                (vectorSnapshotsList[snapI]-interpolant)
            );


            scalar absolute_interpolation_error =MOR::L_infinity_norm(residual);
            snapshotsMeasure = MOR::L_infinity_norm(vectorSnapshotsList[snapI]);
            ...
            {
                tmp_residual_Ptr-> ~GeometricField<vector, fvPatchField, volMesh>();
                max_absolute_error=absolute_interpolation_error;
                tmp_residual_Ptr = new(tmp_residual_Ptr) GeometricField<vector, fvPatchField, volMesh>
                (
                    residual
                );
            }
        ...
        if ((max_relative_error>desidered_error) && (iterIndex<=maxBasis))
        {
            ...
            {
                if (vectorMagnitudeAtMagicPoint< mag((*tmp_residual_Ptr)[CellI]))
                {
                    vectorMagnitudeAtMagicPoint=mag((*tmp_residual_Ptr)[CellI]);
                    tmp_CellID=CellI;
                }
            }
            ...
            *tmp_residual_Ptr/=vectorMagnitudeAtMagicPoint;
    ...
    /* create MagicPointField and write in Constant*/
